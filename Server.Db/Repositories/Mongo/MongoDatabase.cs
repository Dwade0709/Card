...
{
    internal class MongoDatabase<T> where T : IEntity<string>
    {
        ...
        private static string GetConnectionNameFromType()
        {
            ...
            {
                if (typeof(Entity<T>).GetTypeInfo().IsAssignableFrom(entitytype))
                {
                    // No attribute found, get the basetype
                    while (!entitytype.GetTypeInfo().BaseType.Equals(typeof(Entity<T>)))
                    {
